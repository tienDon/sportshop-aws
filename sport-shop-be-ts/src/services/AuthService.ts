...
class AuthService {
  ...
  static async requestOTP(data: RequestOTPData) {
    try {
      ...
      if (name) {
        ...
        // Trong Prisma, email là bắt buộc và unique. Phone là optional.
        // Nếu identifier là phone, ta vẫn cần email giả hoặc xử lý logic khác.
        // Tuy nhiên schema hiện tại: email String @unique, phone String?
        // Nếu đăng ký bằng phone, ta cần email.
        // Giả sử logic cũ cho phép đăng ký bằng phone mà không cần email?
        // Xem lại schema: email String @unique. Vậy bắt buộc phải có email.
        // Nếu identifier là phone, ta không thể tạo user nếu không có email.
        // Tạm thời giả định identifier là email nếu đăng ký.
        // Nếu identifier là phone, ta cần yêu cầu email hoặc sinh email giả.
        ...
        const userData: any = {
          name,
          email: identifier,
          password: "", // Password field is required in schema but we use OTP. Set dummy or handle schema change.
          // Schema: password String. We should probably make it optional or set a random hash.
        };

        // Set dummy password hash
        userData.password = "$2b$10$DummyPasswordHashForOTPUser123456";
        ...
        // Gửi OTP
        const otpResult = await OTPService.generateAndSendOTP(user.id, identifier);
  ...
  static async verifyOTPAndLogin(data: VerifyOTPData) {
    try {
      ...
      let decoded: any;
      try {
        decoded = jwt.verify(otpToken, process.env.JWT_ACCESS_SECRET || "access_secret_key");
      } catch (err) {
        return {
          success: false,
      ...
      // Verify OTP Code
      const verifyResult = await OTPService.verifyOTP(userId, identifier, otpCode);
